...
const questions: Question[] = [
  // Header data fields
  {
    ...
    type: "text" as "text" | "select" | "radio" | "checkbox-multi" | "textarea",
  ...
  // 1 ORGANIZZAZIONE DEL LAVORO
  {
    ...
    type: "select",
    question: "Ore di lavoro settimanali a VDT (abituali)",
    options: ["<20", ">20"],
  },
  {
    id: "1.2",
    section: "1) ORGANIZZAZIONE DEL LAVORO",
    type: "radio",
    question: "La mansione prevede pause/cambi attività di 15 minuti ogni 120 minuti di applicazione continuativa al VDT",
    options: ["SI", "NO"],
  },
  { id: "1.2_note", section: "1) ORGANIZZAZIONE DEL LAVORO", type: "text", question: "Necessità di intervento (eventuale)" },
  {
    ...
    section: "1) ORGANIZZAZIONE DEL LAVORO",
    type: "checkbox-multi",
    question: "Tipo di lavoro prevalente",
  ...
  },
  {
    ...
    section: "1) ORGANIZZAZIONE DEL LAVORO",
    type: "radio",
    question: "È stata effettuata informazione al lavoratore per il corretto uso del VDT",
    options: ["SI", "NO"],
  },
  ...
  // 2 MICROCLIMA
  {
    ...
    type: "radio",
    question: "Modalità per il ricambio d'aria dell'ambiente",
  ...
  },
  {
    ...
    section: "2) MICROCLIMA",
    type: "radio",
    question: "Possibilità di regolare la temperatura dell'ambiente",
  ...
  },
  {
    ...
    section: "2) MICROCLIMA",
    type: "radio",
    question: "Possibilità di regolare l'umidità dell'ambiente",
    options: ["presente", "non presente"],
  },
  {
    id: "2.4",
    section: "2) MICROCLIMA",
    type: "radio",
    question: "Le attrezzature in dotazione producono eccesso di calore che comporta discomfort termico",
    options: ["SI", "NO"],
  },
  ...
  { id: "4.1", section: "4) RUMORE AMBIENTALE", type: "text", question: "Eventuale misura (dB(A))" },
  {
    ...
    type: "radio",
    question: "Può disturbare l'attenzione e la comunicazione verbale",
    options: ["SI", "NO"],
  },
  ...
  // 5 SPAZIO
  {
    ...
    section: "5) SPAZIO",
    type: "radio",
    question: "Spazio di lavoro e manovra adeguato per ruotare/assumere posture",
    options: ["SI", "NO"],
  },
...
const RepartiComparison = ({ filteredResponses, availableCompanies, availableSites, selectedQuestionnaire }: RepartiComparisonProps) => {
  ...
  const reparti = useMemo(
    () => Array.from(new Set(displayedResponses.map((r) => String(r.answers?.meta_reparto || "Sconosciuto")).filter(Boolean))).sort(),
    [displayedResponses]
  );
      ...
      {(availableCompanies.length > 0 || filteredSites.length > 0) && (
        <Card className="shadow-md">
          <CardHeader className="bg-muted/50">
            ...
            <CardDescription>Seleziona azienda e sede per confrontare i reparti</CardDescription>
          </CardHeader>
          <CardContent className="pt-6 space-y-4">
            ...
            {selectedCompany !== "all" && (
              <div className="text-sm text-muted-foreground">
                Confronto reparti per: {availableCompanies.find((c) => c.id === selectedCompany)?.name}
                {selectedSite !== "all" && ` - ${filteredSites.find((s) => s.id === selectedSite)?.name}`}
              </div>
            )}
          </CardContent>
        </Card>
