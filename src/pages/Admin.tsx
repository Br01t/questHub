              ...
              <CardContent className="pt-6">
                <div className="flex gap-3">
                  <div className="flex-1 space-y-2">
                    ...
                    <Input id="company-name" value={newCompanyName} onChange={(e) => setNewCompanyName(e.target.value)} placeholder="Es: Acme Corp" />
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input id="site-name" value={newSiteName} onChange={(e) => setNewSiteName(e.target.value)} placeholder="Es: Sede di Milano" />
                    </div>
                    <div className="space-y-2">
                        ...
                        <div key={site.id} className="p-4 border rounded-lg hover:bg-accent/5 transition-colors">
                          ...
                          {assignedUsers.length > 0 ? (
                            <div className="mt-3 pl-8 border-l-2 border-primary/20">
                              <p className="text-sm font-medium mb-1 text-muted-foreground">Utenti assegnati:</p>
                              <ul className="space-y-1">
                                {assignedUsers.map((user) => (
                                  <li key={user.userId} className="text-sm flex items-center gap-2">
                            ...
                            <p className="mt-3 text-xs text-muted-foreground italic pl-8">Nessun utente assegnato</p>
                          )}
                        </div>
                      );
                    })}
          ...
          <TabsContent value="users" className="space-y-6">
            <Card className="shadow-xl border-2">
              <CardHeader className="border-b">
                ...
                <CardDescription>Assegna ruoli, aziende e sedi agli utenti</CardDescription>
              </CardHeader>
                  ...
                  <div className="space-y-3">
                    {filteredUsers.map((userProfile) => {
                      ...
                      const toggleCompany = (companyId: string) => {
                        const updatedCompanies = userState.companies.includes(companyId)
                      ...
                      const confirmAssignments = async () => {
                        try {
                          await updateDoc(doc(db, "userProfiles", userProfile.userId), {
                      ...
                      return (
                        <div key={userProfile.userId} className="p-4 border rounded-lg space-y-4">
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              ...
                              <Button
                                variant="destructive"
                                size="sm"
                                onClick={() => deleteUserProfile(userProfile.userId, userProfile.email)}
                                className="gap-2 w-40 justify-center"
                              >
                                <Trash2 className="h-4 w-4" />
                                Elimina
                            ...
                            <div className="border rounded-lg p-3 bg-muted/10 space-y-1">
                              {companies.map((company) => (
                                <div key={company.id} className="flex items-center space-x-2">
                                  <input
                                    type="checkbox"
                                    checked={userState.companies.includes(company.id)}
                                    onChange={() => toggleCompany(company.id)}
                                    id={`${userProfile.userId}-${company.id}`}
                                  />
                                  <Label htmlFor={`${userProfile.userId}-${company.id}`} className="cursor-pointer text-sm">
                                    {company.name}
                                  </Label>
                                </div>
                              ))}
                          ...
                          {userState.companies.map((companyId) => {
                            ...
                            const relatedSites = sites.filter((s) => s.companyId === companyId);
                            ...
                            return (
                              <div key={companyId} className="space-y-2">
                                <Label className="text-xs">
                                  Sedi di <span className="font-medium">{company.name}</span>
                                </Label>
                                <div className="border rounded-lg p-3 bg-muted/10 space-y-1">
                                  {relatedSites.length > 0 ? (
                                    relatedSites.map((site) => (
                                      <div key={site.id} className="flex items-center space-x-2">
                                        <input
                                          type="checkbox"
                                          checked={userState.sites.includes(site.id)}
                                          onChange={() => toggleSite(site.id)}
                                          id={`${userProfile.userId}-${site.id}`}
                                        />
                                        <Label htmlFor={`${userProfile.userId}-${site.id}`} className="cursor-pointer text-sm">
                                          {site.name} – <span className="text-xs text-muted-foreground">{site.address}</span>
                                        </Label>
                                      </div>
                                    ))
                                  ) : (
                                    <p className="text-xs text-muted-foreground italic">Nessuna sede per questa azienda</p>
                                  )}
                                </div>
                              </div>
                            );
                          ...
                          {userState.pending && (
                            <div className="pt-3 flex justify-end">
                              <Button size="sm" className="gap-2" onClick={confirmAssignments}>
